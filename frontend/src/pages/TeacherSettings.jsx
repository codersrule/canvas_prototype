...
const STORAGE_KEY = 'classroom_teacher_settings'
...
export function TeacherSettingsPage() {
  ...
  const displayInitials = user?.initials || displayName.split(/\s+/).map((s) => s[0]).join('').slice(0, 2).toUpperCase() || '?'
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-600 mt-1">
          Manage your profile, teaching preferences, and notifications.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <section className="bg-white rounded-lg shadow-md p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Schedule Office Hours</h2>
          <p className="text-xs text-gray-500 mb-3">
            Set your office hours so students know when they can reach you.
          </p>
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1">Office hours</label>
            <input
              type="text"
              value={settings.officeHours ?? ''}
              onChange={(e) => handleChange('officeHours', e.target.value)}
              placeholder="e.g. Tuesday 2-4pm, Thursday 3-5pm"
        ...
        <section className="bg-white rounded-lg shadow-md p-6">
          ...
          <p className="text-xs text-gray-500 mb-3">
            Choose how you&apos;d like to be notified about submissions, grading, and student activity.
          </p>
          <div className="space-y-2">
            ...
            <Toggle
              ...
              onChange={(v) => handleChange('submissionAlerts', v)}
            />
            <Toggle
              id="gradingReminders"
              label="Grading reminders"
              description="Reminders when assignments have pending grades."
              checked={settings.gradingReminders}
              onChange={(v) => handleChange('gradingReminders', v)}
            />
            <Toggle
              id="studentMessages"
              label="Student messages"
              description="Notifications for new inbox messages from students."
              checked={settings.studentMessages}
              onChange={(v) => handleChange('studentMessages', v)}
            />
          </div>
